...
public class StyleRadioGroup extends LinearLayout {
     ...
     * {@inheritDoc}
     */
    public StyleRadioGroup(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public StyleRadioGroup(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
        setOrientation(VERTICAL);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.StyleRadioGroup, defStyleAttr, 0);
        String names = ta.getString(R.styleable.StyleRadioGroup_srg_radio_btn_names);
        String defaultChecked = ta.getString(R.styleable.StyleRadioGroup_srg_default_checked);
        if (names != null && names.length() != 0) {
            String[] nameArray = names.split(",");
            int lines = nameArray.length / 3;
            int mod = nameArray.length % 3;

            if (lines < 1) {
                lines = 1;
            }

            if (mod != 0 && nameArray.length > 3) {
                lines += 1;
            }
            for (int i = 0; i < lines; i++) {
                ...
                for (int j = 0; j < 3; j++) {
                    ...
                    if (index > nameArray.length - 1) {
                        ...
                        if (name.equals(defaultChecked)) {
                            rb.setChecked(true);
                        }
                        rb.setText(name);
                    }
                    rb.setTextColor(getResources().getColorStateList(R.color.color_list_blue_black));
                    rb.setBackgroundResource(R.drawable.selector_rectangle_frame);
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(StyleRadioGroup.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(StyleRadioGroup.class.getName());
    }
    ...
    public interface OnCheckedChangeListener {
         ...
         */
        public void onCheckedChanged(StyleRadioGroup group, int checkedId);
    }
    ...
    private class PassThroughHierarchyChangeListener implements
        ...
        public void onChildViewAdded(View parent, View child) {
            if (parent == StyleRadioGroup.this) {
                List<RadioButton> btns = getAllRadioButton(child);
                if (btns != null && btns.size() > 0) {
