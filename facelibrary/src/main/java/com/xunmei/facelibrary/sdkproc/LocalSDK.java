...
public class LocalSDK {
	...
	// 1：N人脸检索
	public RecogBean Recog(byte[] btFeaProbe, byte[] btFeaFiled, int iFiledNum)
	{
		...
		// 1:n
		recogBean.ret = recog.cwComputeMatchScore(iRecogHandle, btFeaProbe, iFeaLen, 1, btFeaFiled, iFeaLen, iFiledNum, recogBean.scores);
		return recogBean;
	}
	...
	public AttributeBean GetAttriFromImgData(byte[] imgData)
	{
		...
		attriBean.ret = FaceDetectBase(imgData, 0, 0, FaceInterface.cw_img_form_t.CW_IMAGE_BINARY, 0, 0,
				FaceInterface.cw_op_t.CW_OP_DET | FaceInterface.cw_op_t.CW_OP_ALIGN, pFaceBuffer);
		if (attriBean.ret >= ERRCODE_MIN) {
			...
			attriBean.ret = attri.cwGetAgeGenderEval(attributeChannel, pFaceBuffer[0].alignedData, pFaceBuffer[0].alignedW,
