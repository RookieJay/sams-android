...
public class NisLivenessSDK {
	...
	public int CreateNisHandle(String pLicence, String sModelDir) {
		...
		String sModelPath = new StringBuilder(sModelDir).append(File.separator).append("nirLivDet_model.bin").toString();
		String sMatrixParaDir = new StringBuilder(sModelDir).append(File.separator).append("matrix_para.xml").toString();
		// log保存文件夹路径
		String sLogDir = Environment.getExternalStorageDirectory() + File.separator + "cwlivlog";
		float fThresh = 0.5f;    // 肤色阈值
		
		int ret = m_faceLive.cwCreateNirLivenessHandle(sModelPath, sMatrixParaDir, sLogDir, fThresh, 
				FaceInterface.cw_nirliv_camera_type_t.CW_NIR_LIV_DET_BINOCULAR,
				FaceInterface.cw_nirliv_det_type_t.CW_NIR_LIV_MODE_MULTI_FACES, pLicence);
	...
	public NisLiveBean FaceNirLiveDetect(byte[] dataVis, int widthVis, int heightVis, int formatVis, 
		...
		float[] visKeyPtScore = new float[1];
		float[] visSkinScore = new float[1];
		float[] visKeypt_x = new float[iKeyPtNum];
		float[] visKeypt_y = new float[iKeyPtNum];
		visKeyPtScore[0] = detBeanVis.faceInfos[0].keyptScore;
		visSkinScore[0] = detBeanVis.faceInfos[0].scores[8];
		...
		// 只取第一个人脸
		float[] nirKeyPtScore = new float[1];
		float[] nirSkinScore = new float[1];
		float[] nirKeypt_x = new float[iKeyPtNum];
		float[] nirKeypt_y = new float[iKeyPtNum];
		nirKeyPtScore[0] = detBeanNir.faceInfos[0].keyptScore;
		nirSkinScore[0] = detBeanNir.faceInfos[0].scores[8];
