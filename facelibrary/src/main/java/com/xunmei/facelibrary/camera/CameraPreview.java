...
import com.xunmei.facelibrary.realtime.CaremaType;
import com.xunmei.facelibrary.realtime.LocalFaceSDK;
        ...
        if (caremaId == Camera.CameraInfo.CAMERA_FACING_FRONT) {// 前置
            if (Configuration.ORIENTATION_PORTRAIT == orientation) {// 竖屏
                ...
                LocalFaceSDK.getInstance(context).cwPushFrame(data, reqPrevW, reqPrevH,
                        FaceInterface.cw_img_form_t.CW_IMAGE_NV21, CaremaType.FRONT_PORTRAIT,camera);
            } else {//横屏水平镜像
                LocalFaceSDK.getInstance(context).cwPushFrame(data, reqPrevW, reqPrevH,
                        FaceInterface.cw_img_form_t.CW_IMAGE_NV21, CaremaType.FRONT_LANDSCAPE,camera);
            }
        } else {// 后置
            if (Configuration.ORIENTATION_PORTRAIT == orientation) {// 竖屏 旋转90
                LocalFaceSDK.getInstance(context).cwPushFrame(data, reqPrevW, reqPrevH,
                        FaceInterface.cw_img_form_t.CW_IMAGE_NV21, CaremaType.BACK_PORTRAIT,camera);
            } else {
                // 横屏不做处理
                LocalFaceSDK.getInstance(context).cwPushFrame(data, reqPrevW, reqPrevH,
                        FaceInterface.cw_img_form_t.CW_IMAGE_NV21, CaremaType.BACK_LANDSCAPE,camera);
            }
        }
